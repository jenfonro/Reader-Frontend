...
export const simplified = function(orgStr) {
  let str = "";
  for (let i = 0; i < orgStr.length; i++) {
    const char = orgStr[i];
    if (char.charCodeAt(0) > 10000) {
...
export const normalizeChineseMode = mode => (mode === "繁体" ? "繁体" : "简体");

export const convertChineseText = (text, mode = "简体") => {
  const source = String(text ?? "");
  return normalizeChineseMode(mode) === "繁体" ? traditionalized(source) : simplified(source);
};
